...
import { Mail, MapPin, Phone, Sprout, Check } from 'lucide-react';
import { CONTACT_INFO } from '../../../data/landingContent';
import { geoNav, geoServices } from '../../../data/geoLandingData';
...
export const GeoFooter=()=><footer className="geo-footer"><SectionContainer>
 ...
 <div><h3>Liên kết nhanh</h3>{geoNav.map(([label,href])=><Link key={label} to={href}>{label}</Link>)}</div>
 ...
 <div className="geo-footer-bottom"><span>© {new Date().getFullYear()} Localmate. All rights reserved.</span><div><Link to="/chinh-sach-bao-mat">Chính sách bảo mật</Link><Link to="/dieu-khoan">Điều khoản sử dụng</Link><Link to="/sitemap">Sitemap</Link></div><span>Made with ♥ for a smarter Vietnam</span></div>
